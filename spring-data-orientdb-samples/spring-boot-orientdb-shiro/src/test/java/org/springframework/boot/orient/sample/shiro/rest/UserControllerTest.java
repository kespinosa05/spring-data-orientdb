...
@SpringBootTest(classes = { Application.class, OrientDbConfiguration.class,
		ShiroConfiguration.class }, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
...
public class UserControllerTest extends AbstractTestNGSpringContextTests {
	...
	public void test_authenticate_success() throws JsonProcessingException {
		...
		final ResponseEntity<String> response = new TestRestTemplate(TestRestTemplate.HttpClientOption.ENABLE_COOKIES)
				.exchange(BASE_URL.concat("/auth"), HttpMethod.POST, new HttpEntity<>(json, headers), String.class);
	...
	public void test_authenticate_failure() throws JsonProcessingException {
		...
		final String json = new ObjectMapper()
				.writeValueAsString(new UsernamePasswordToken(USER_EMAIL, "wrong password"));
		System.out.println(json);
		final ResponseEntity<String> response = new TestRestTemplate(TestRestTemplate.HttpClientOption.ENABLE_COOKIES)
				.exchange(BASE_URL.concat("/auth"), HttpMethod.POST, new HttpEntity<>(json, headers), String.class);
